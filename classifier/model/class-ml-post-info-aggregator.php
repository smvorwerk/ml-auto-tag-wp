...
class Post_Info_Aggregator {
	...
	public function __toString() :string { // quick and dirty debugging method
		$o = "";
		$o .= "feature data: <br>";
	...
	private function clean_frequency_feature(String $feature) {
		//E.g. "<p>You're tearing me <i>apart</i>, Lisa!</p>" --> "youre tearing me apart lisa"
	...
	private function clean_categorical_feature(String $feature) {
		...
		//E.g. "Walt Whitman" => "Walt_Whitman", "My Brother's Posts" => "My_Brother_s_Posts"
		return preg_replace('/[^A-z0-9]+/', '_', $feature);
	}
	...
	private function extract_post_features(Object &$post, array $feature_names) {
		...
		foreach($feature_names as $feature_type) {
			if (in_array($feature_type, $frequncy_features)) //Clean frequency features
				$cleaned_feature = $this->clean_frequency_feature($post->$feature_type);
			if (in_array($feature_type, $categorical_features)) { //Clean categorical features
	...
	private function extract_post_attributes(array $taxonomies, array $feature_names, int $post_id): void {
		...
		$args = [ 'numberposts' => -1 ];
		if ($post_id) { //If we're only retrieving one post
			$args["include"] = array($post_id);
			$args["numberposts"] = 1;
		}
		$all_posts = get_posts( $args );
		foreach ($all_posts as $post) {
			$this->extract_post_features($post, $feature_names); //push features to array in order of post
